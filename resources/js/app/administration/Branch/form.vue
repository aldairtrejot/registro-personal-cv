    ...
    <div class="card shadow-lg">
        <div class="card-body">
            ...
            <form role="form" id="data_form" enctype="multipart/form-data">
                ...
                <footerButton :cancelUrl="`${BASE_URL}/role/branch`" :onSave="send_data_form" />
            </form>
        </div>
    </div>
</template>
...
onMounted(async () => {
    try {
        ...
        const response = await axios.post('/role/branch/main', { id: key }) // <- solo si decides hacer todo en Vue
...
async function send_data_form() {
    try {
        ...
        const response = await axios.post('/role/branch/save', formData)
        ...
        if (!response.data.status) {
            ...
            window.location.href = `${BASE_URL}/role/branch`
        }
