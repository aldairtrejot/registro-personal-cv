  ...
  <div class="cv-wrapper">
    <div class="cv-card shadow-sm">
      ...
      <section class="cv-content">
        <!-- PASO 1: CURP + CORREO (SIN TOKEN) -->
        <div v-if="pasoActual === 1">
          <h3 class="cv-section-title">1. Acceso</h3>
          ...
          <div class="row g-3 mt-1">
            ...
            <div class="col-12">
              ...
              <input v-model.trim="form.token" class="form-control" />
            </div>
            -->
          </div>
        ...
        <div v-else-if="pasoActual === 3">
          <h3 class="cv-section-title">3. Experiencia laboral</h3>
          ...
          <div v-for="(exp, index) in form.experiencias" :key="index" class="cv-block">
            ...
            <div class="row g-3">
              ...
              <div class="col-12">
                ...
                <input v-model.trim="exp.puesto" @input="exp.puesto = toUpperText(exp.puesto)" type="text" class="form-control" maxlength="150" />
              </div>

              <div class="col-12">
                <label class="form-label">Institución</label>
                <input v-model.trim="exp.institucion" @input="exp.institucion = toUpperText(exp.institucion)" type="text" class="form-control" maxlength="200" />
              </div>

              <div class="col-12">
                <label class="form-label">Campo de experiencia</label>
                <input v-model.trim="exp.campo" @input="exp.campo = toUpperText(exp.campo)" type="text" class="form-control" maxlength="100" />
          ...
          <div class="cv-actions cv-actions-two">
            <button type="button" class="btn btn-outline-secondary" :disabled="loading" @click="irPaso(2)">← Volver</button>
        ...
        <div v-else-if="pasoActual === 4">
          <h3 class="cv-section-title">4. Estudios académicos</h3>

          <div class="row g-3 mt-1">
            <div class="col-12">
              <label class="form-label">Institución</label>
              <input v-model.trim="form.estudios.institucion" @input="form.estudios.institucion = toUpperText(form.estudios.institucion)" type="text" class="form-control" maxlength="200" />
            </div>
            ...
            <div class="col-12">
              ...
              <select v-model="form.estudios.id_pais" class="form-select" @change="syncPaisTexto">
                <option :value="null">Selecciona…</option>
                <option v-for="pais in catalogos.paises" :key="pais.id" :value="pais.id">{{ pais.nombre }}</option>
              </select>
            </div>
            ...
            <div class="col-12">
              ...
              <select v-model="form.estudios.id_nivel_estudios" class="form-select" @change="syncNivelTexto">
                <option :value="null">Selecciona…</option>
                <option v-for="nivel in catalogos.nivelesEstudio" :key="nivel.id" :value="nivel.id">{{ nivel.nombre }}</option>
              </select>
            </div>

            <div class="col-12">
              <label class="form-label">Número de cédula</label>
              <input v-model.trim="form.estudios.numero_cedula" @input="form.estudios.numero_cedula = toUpperText(form.estudios.numero_cedula)" type="text" class="form-control" maxlength="50" />
            </div>
            ...
            <div class="col-12">
              ...
              <select v-model="form.estudios.id_carrera_especifica" class="form-select" @change="onChangeCarreraEspecifica">
                <option :value="null">Selecciona…</option>
                <option v-for="car in catalogos.carrerasEspecificas" :key="car.id" :value="car.id">{{ car.nombre }}</option>
              </select>
            </div>
          ...
          <div class="cv-actions cv-actions-two">
            <button type="button" class="btn btn-outline-secondary" :disabled="loading" @click="irPaso(3)">← Volver</button>
          ...
          <div v-for="(curso, index) in form.cursos" :key="index" class="cv-block">
            ...
            <div class="row g-3">
              <div class="col-12">
                ...
                <input v-model="curso.fecha_inicio" type="date" class="form-control" @change="syncCursoPeriodo(curso)" />
              </div>

              <div class="col-12">
                <label class="form-label">Fecha fin</label>
                <input v-model="curso.fecha_fin" type="date" class="form-control" @change="syncCursoPeriodo(curso)" />
              </div>

              <div class="col-12">
                <label class="form-label">Período (generado)</label>
                <input v-model="curso.periodo" type="text" class="form-control" readonly placeholder="DD/MM/AAAA - DD/MM/AAAA" />
              </div>

              <div class="col-12">
                <label class="form-label">Nombre del curso</label>
                <input v-model.trim="curso.nombre" @input="curso.nombre = toUpperText(curso.nombre)" type="text" class="form-control" maxlength="200" />
              </div>

              <div class="col-12">
                <label class="form-label">Institución</label>
                <input v-model.trim="curso.institucion" @input="curso.institucion = toUpperText(curso.institucion)" type="text" class="form-control" maxlength="200" />
              </div>
            </div>
          </div>

          <button type="button" class="btn btn-link p-0 mt-2 cv-link-add" :disabled="form.cursos.length >= 5" @click="agregarCurso">
            + Agregar otro curso (máx. 5)
          </button>

          <div class="cv-actions cv-actions-three">
            <button type="button" class="btn btn-outline-secondary" :disabled="loading" @click="irPaso(4)">← Volver</button>
  ...
  methods: {
    ...
    async cargarCatalogos() {
      try {
        ...
        const [paisesRes, nivelesRes, areasRes, puestosRes, puestosEspRes, unidadesRes, carrerasEspRes] = results
    ...
    async guardarExperiencias() {
      ...
      try {
        ...
        await axios.post('api/cv/experiencias', { curp: this.form.curp, experiencias: this.form.experiencias })
    ...
    async guardarEstudios() {
      ...
      try {
        ...
        await axios.post('api/cv/estudios', { curp: this.form.curp, ...this.form.estudios })
