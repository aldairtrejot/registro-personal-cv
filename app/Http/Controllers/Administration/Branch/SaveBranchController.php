...
class SaveBranchController extends Controller
{
    /**
    ...
    public function save(Request $request)
    {
        try {
            ...
            return response()->json([
                'status' => false,
                'message' => 'Ocurrió un error al guardar la rama.',
            ], 200);
        }
    }
    /**
    ...
    private function storage($request)
    {
        try {
            ...
            if ($request->filled('id_cat_rama')) {
                // Editar
                $model::where('id_cat_rama', $request->id_cat_rama)->update($data);
                $message = 'Rama actualizada correctamente.';
            } else {
                // Crear
                $data['creado_en'] = $timestamp;
                $data['id_usuario_creacion'] = Auth::user()->id;
                $model::create($data);
                $message = 'Rama guardada correctamente.';
            }
            return response()->json([
                'status' => true,
                'message' => $message,
